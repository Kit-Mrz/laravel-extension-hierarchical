...
class BatchStoreRequest extends TemplateAbstract implements TemplateCreatorContract
{
    use TemplateTool;
    ...
    public function handle() : array
    {
        ...
        $parser             = new NewTableParser(new TableInformation($this->getTableName(), $this->getTablePrefix()));
        $templateRender     = $parser->getStoreTemplateRender($this->getIgnoreFields(), true);
        $ruleStoreString    = $templateRender->getRuleString();
        $messageStoreString = $templateRender->getMessageString();
        ...
        $replacementRules = [
            ...
            '/{{RNT}}/'            => $controlName,
            '/{{RULE_STRING}}/'    => $ruleStoreString,
            '/{{MESSAGE_STRING}}/' => $messageStoreString,
        ];
